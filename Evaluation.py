...
if __name__ == "__main__":
    ...
    for filename in data_info['chan_id']:
        if filename != '.json':
            ...
            new_row = pd.Series(
                [filename, tp, tn, fp, fn, roc_auc, pr_auc, best_tp, best_tn, best_fp, best_fn, best_pre, best_rec,
                 best_f1],
                index=['name', 'tp', 'tn', 'fp', 'fn', 'roc', 'pr', 'best_tp', 'best_tn', 'best_fp', 'best_fn',
                       'best_pre', 'best_rec', 'b_f_1'])
    ...
    print(f"Results saved to {ds_name}_results_woincon.csv")