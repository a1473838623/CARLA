...
class SMAP(Dataset):
    ...
    def __init__(self, fname, root=MyPath.db_root_dir('smap'), train=True, transform=None, sanomaly=None,
        ...
        if fname == 'All':
            # 新格式：加载合并的单文件
            if self.train:
                file_path = os.path.join(self.root, 'SMAP_train.npy')
                temp = np.load(file_path)
                labels = np.zeros(len(temp))
            else:
                file_path = os.path.join(self.root, 'SMAP_test.npy')
                temp = np.load(file_path)
                label_path = os.path.join(self.root, 'SMAP_test_label.npy')
